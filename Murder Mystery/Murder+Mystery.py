...
def get_average_sentence_length(text):
    text2 = text.replace("!", ".")
    text3 = text2.replace("?", ".")
    sentences_in_text = text3.split(".")
    while "" in sentences_in_text:
        sentences_in_text.remove("")
    sentence_lengths = 0
    for sentence in sentences_in_text:
        sentence_lengths +=len(sentence.strip(" ").split(" "))
    return sentence_lengths / (len(sentences_in_text))
...
class TextSample:
    ...
    def __repr__(self):
        return "The author's name is {a} and the average sentence length is {s}".format(a=self.author,s=self.average_sentence_length)
...
murderer_sample = TextSample(text=murder_note, author="unknown")
lily_sample = TextSample(text=lily_trebuchet_intro, author="Trebuchet")
myrtle_sample = TextSample(text=myrtle_beech_intro, author="Beech")
gregg_sample = TextSample(text=gregg_t_fishy_intro, author="Fishy")
...
def find_text_similarity(argument1, argument2):
    ...
    ngram_similarity = frequency_comparison(build_frequency_table(argument1.ngram_frequency), build_frequency_table(argument2.ngram_frequency))
...
print("Lily Trebuchet", find_text_similarity(murderer_sample, lily_sample))
print("Myrtle Beech", find_text_similarity(murderer_sample, myrtle_sample))
print("Gregg T. Fishy", find_text_similarity(murderer_sample, gregg_sample))
